...
export function Faq() {
  ...
  const textColorClass =
    colorMode === "light" ? "text-gray-700" : "text-gray-100";
      ...
      <Container className="">
        <Box className="mx-auto max-w-2xl lg:mx-0">
          <Text
            as="h2"
            id="faqs-title"
            className={`text-3xl font-medium tracking-tight ${headingColorClass}`}
          >
            {t("faq_title")}
          </Text>
        </Box>
          ...
          {FAQS.map((_, index) => {
            return (
              <ListItem className="space-y-10" key={`question_${index}`}>
                <Text
                  as="h3"
                  className={`text-lg font-semibold leading-6 ${headingColorClass}`}
                >
