...
import {
  updateUsersDisplayNames,
  updateUsersProfilePictures,
} from "src/lib/leaderboard_utilities";
...
export default withoutRole("banned", async (req, res, token) => {
  ...
  const time_frame =
    (req.query.time_frame as LeaderboardTimeFrame) ?? LeaderboardTimeFrame.day;
  const limit = parseInt(req.query.limit as string);
  const includeUserStats = req.query.includeUserStats === "true";

  if (!includeUserStats) {
    const leaderboardReply = await oasstApiClient.fetch_leaderboard(
      time_frame,
      { limit },
    );
    leaderboardReply.leaderboard = updateUsersDisplayNames(
      leaderboardReply.leaderboard,
    );
    leaderboardReply.leaderboard = await updateUsersProfilePictures(
      leaderboardReply.leaderboard,
    );
  ...
  res.status(200).json({
    ...leaderboardReply,
    user_stats_window: user_stats?.leaderboard.map((stats) => ({
      ...stats,
      is_window: true,
    })),
  });
