...
export const useGenericTaskAPI = <
  TaskType extends BaseTask,
  ResponseContent = AllTaskReplies,
>(
  taskType: TaskTypeEnum,
): TaskApiHook<TaskType, ResponseContent> => {
  const [response, setResponse] = useState<TaskResponse<TaskType>>({
  ...
  const { isValidating: isLoading, mutate: requestNewTask } = useSWRImmutable<
    ServerTaskResponse<TaskType>
  >(API_ROUTES.NEW_TASK(taskType, { lang: locale }), get, {
    onSuccess: (taskResponse) => {
      setResponse({
        ...taskResponse,
        taskAvailability: "AVAILABLE",
        // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
        taskInfo: TaskInfos.find(
          (taskType) => taskType.type === taskResponse.task.type,
        )!,
      });
    },
    onError: () => {
  ...
  const { trigger: sendTaskContent, isMutating: isSubmitting } = useSWRMutation(
    API_ROUTES.UPDATE_TASK,
    post,
    {
      onSuccess: () => {
        requestNewTask();
      ...
      onError: (err: unknown) => {
        if (
          err instanceof OasstError &&
          err.errorCode === ERROR_CODES.TASK_REQUESTED_TYPE_NOT_AVAILABLE
        ) {
          setResponse({ taskAvailability: "NONE_AVAILABLE" });
        } else {
  ...
  const { trigger: sendRejection, isMutating: isRejecting } = useSWRMutation(
    "/api/reject_task",
    post,
    {
      onSuccess: skipTask,
  ...
  const completeTask = useCallback(
    async (content: ResponseContent) => {
      ...
      await sendTaskContent({
        id: response.id,
        update_type: response.taskInfo.update_type,
        content,
        lang: locale,
      });
    },
