...
1.  Node 16: if you are on windows, you can
    [download node from their website](https://nodejs.org/en/download/releases),
    if you are on linux, use [NVM](https://github.com/nvm-sh/nvm) (Once
    installed, run `nvm use 16`)
...
1.  Run
    ...
    You can optionally include `-d` to detach and later track the logs if
    desired.
    - If you want to work on the chat api, you need to run the inference profile
      as well. Your new command would look like:
...
Create a `docker-compose.override.yml` in the root of the repo, and add the
following to it
...
And now when you start all containers, the possibility to login to inference
through these providers will be available.
...
To create a new story create a file named `[componentName].stories.tsx`. An
example how such a story could look like, see `Header.stories.tsx`.
